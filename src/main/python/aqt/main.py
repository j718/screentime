...
class ScreentimeQt(QMainWindow):
    ...
    def setup_tray(self):
        ...
        """
        self.tray = QSystemTrayIcon(self.appctxt.app_icon, self.appctxt.app)
        # tray.setVisible(True)

        # create tray menu
        t_menu = QMenu()
        exit_action = QAction('Exit Application', self.tray)
        exit_action.setStatusTip('Exit the application.')
        exit_action.triggered.connect(lambda: sys.exit(0))
        dashboard_action = QAction('Dashboard', self.tray)
        ...
        self.tray.setContextMenu(t_menu)
        self.tray.show()
